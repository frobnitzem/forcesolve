...
# -- Useful for calculating the quadratic integral matrix.
        def dpoly_integral(self, a, b, n=0, m=0, t=0, s=1.0):
                if n >= self.order:
                        raise ValueError("%d order spline is only %d times "\
...
class spline_func:
        def __init__(self, bin, spl, periodic=False, shift=0, rng=(-inf,inf), id="", verb=True):
                ...
                if self.n < spl.order:
                        raise ValueError("Error! Number of spline points is "\
                ...
                if not periodic: # splines to function correctly.
                        ...
                        if (rng[0]-self.x0)*self.ih+shift > self.spl.order-1:
                            raise ValueError("Lower range of spline "\
                                        "creates meaningless coeff.s")
                        if (rng[1]-self.x0)*self.ih+shift < self.n-1:
                            raise ValueError("Upper range of spline "\
                                        "creates meaningless coeff.s")
                        self.rng = [0,0]
        ...
        def integral(self, n=0, m=0):
                if n != 0:
                        raise NotImplementedError("Error! nonzero values of n not yet supported.")
                M = self.spl.M
                r = self.spl.order
...
def read_spline_func(file, name=None):
        ...
        if info[0] != "SPLINE" or len(info) != 17:
                raise TypeError("Error! Input file is not SPLINE type.")
        if name == None:
                id = info[1]
        else:
                id = name
