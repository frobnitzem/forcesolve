...
class cspline:
        def __init__(self, dat, mode=2, a=0., b=0.):
                if(dat.shape[1] < 2):
...
class cspline_h:
        ...
        def yp(self, x): # quadratic interpolation
                ...
                a = self.h*(3*a*a-1.0)/6.0
                b = self.h*(3*b*b-1.0)/6.0
                
                yp = (self.spl[u,1]-self.spl[l,1])/self.h
                yp += b*self.spl[u,2]-a*self.spl[l,2]
...
def cspline_file(filename, x_col=0, y_col=1, mode=2, e_col=-1, a=0., b=0.):
        ...
        n = data.shape[0]
        if(data.shape[1] < 2):
                raise ValueError("Error! Data file must have >1 col.")
...
def spline_table(dat, mode, a, b):
        if(mode != 1 and mode != 2):
                raise ValueError("Unacceptable value entered for mode.")
